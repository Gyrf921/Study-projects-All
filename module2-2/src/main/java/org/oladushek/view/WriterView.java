...
public class WriterView {
    ...
    public void handle() {
        ...
        while (running) {
            ...
            System.out.println("7. Add existing posts for writer");
            System.out.println("8. Delete writer");
            ...
            switch (choice) {
                ...
                case "4" -> createEmpty();
                case "5" -> update();
                case "6" -> createNewPostForWriter();
                case "7" -> addExistingPost();
                case "8" -> delete();
                case "9", "exit", "Exit" -> running = false;
    ...
    private void createNewPostForWriter() {
        System.out.print("Enter writer ID for new post: ");
        Long id = Long.parseLong(scanner.nextLine());

        System.out.print("Enter post content: ");
        String content = scanner.nextLine();

        WriterDTO updated = writerController.createNewPost(id, content, labelsIdForNewPost());
        if (updated != null) {
            System.out.println("Updated: " + updated);
        } else {
            System.out.println("Writer not found.");
    ...
    private void addExistingPost() {
        System.out.print("Enter writer ID for add post: ");
        Long id = Long.parseLong(scanner.nextLine());

        WriterDTO updated = writerController.update(id, "exit", "exit", postsIdForNewWriter());
        if (updated != null) {
            System.out.println("Updated: " + updated);
        } else {
            System.out.println("Writer not found.");
    ...
    private List<Long> postsIdForNewWriter() {
        ...
        postController.getAllWithoutWriter().forEach(System.out::println);
